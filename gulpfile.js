...
async function injectFolderBreadcrumb(outputContent, file) {
  ...
  do {
    ...
    if (num > 1) upHref = upHref + "../";
    let html;
    if (num == 0) html = "<span>" + folderName + "</span>";
    else html = '<a href="' + upHref + '"><span>' + folderName + "</span></a>";
    breadcrumbHtmlArr.push(html);
    currentDir = currentDir.substring(0, currentDir.lastIndexOf(path.sep));
    num++;
  } while (currentDir != opusRoot);
...
async function generateIndex(dirPath) {
  ...
  for (const item of fsItems) {
    if (item.isDirectory()) {
      ...
      if (itemName == "index.json") continue;
...
export function make_opus_index() {
  return src("src/opus/**/", { read: false }).pipe(
...
export const sync_deploy = async () => {
  let profile = "sample";
